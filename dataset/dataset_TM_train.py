...
class Text2MotionDataset(data.Dataset):
    def __init__(self, dataset_name, feat_bias = 5, unit_length = 4, codebook_size = 1024, tokenizer_name=None, up_low_sep=False, mode=None):
        ...
        new_name_list = []
        data_dict = {}
        id_list = id_list[:666]  if sys.gettrace() else id_list
        if mode == 'debug':
            id_list = id_list[6:7]
...
def DATALoader(dataset_name,
                ...
                num_workers = 8, up_low_sep=False, mode=None) : 

    train_loader = torch.utils.data.DataLoader(Text2MotionDataset(dataset_name, codebook_size = codebook_size, tokenizer_name = tokenizer_name, unit_length=unit_length, up_low_sep=up_low_sep, mode=mode),
                                              batch_size,
                                              shuffle=True,
