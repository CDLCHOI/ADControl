...
from utils import motion_process
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        '''
        mu_t(xt, x0) = self.posterior_mean_coef1 * x0 + self.posterior_mean_coef2 * xt
        beta_t = self.posterior_variance
        '''
    ...
    def q_mean_variance(self, x_start, t):
        """
        获取一次性前向的均值和方差, 也就是公式(2)的乘加系数  Get the distribution q(x_t | x_0).
    ...
    def p_mean_variance(
        ...
        :param clip_denoised: if True, clip the denoised signal into [-1, 1]. 噪声截断
        ...
        model_output = model(x, self._scale_timesteps(t), model_kwargs['text_emb'], word_emb=model_kwargs['word_emb']) 

        # if 'inpainting_mask' in model_kwargs['y'].keys() and 'inpainted_motion' in model_kwargs['y'].keys(): # 没走这里
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]: # self.model_var_type = FIXED_SMALL
        ...
        '''
        model_mean_type表示网络学习的是什么东西，有三种类型：x_{t-1}、x0、噪声
        1. 若网络输出是x_{t-1}, 根据DDPM论文公式(7) mu_t = x_{t-1} = coef1*x0 + coef2*xt, 得x0 = x_{t-1}/coef1 - (coef2/coef1) * xt
        2. 若网络输出是x0, 不处理
        3. 若网络输出是噪声, x_{t-1} = coef1*x0 + coef2*xt 带入 xt = self.sqrt_alphas_cumprod*x0 + (1-self.alphas_cumprod)*eps
        '''
        if self.model_mean_type == ModelMeanType.PREVIOUS_X: # 均值类型为x_{t-1}，即网络直接预测x_{t-1}
            pred_xstart = process_xstart(
                self._predict_xstart_from_xprev(x_t=x, t=t, xprev=model_output)
            )
            model_mean = model_output
        elif self.model_mean_type in [ModelMeanType.START_X, ModelMeanType.EPSILON]:  # THIS IS US!
            if self.model_mean_type == ModelMeanType.START_X: # ModelMeanType.START_X意思是均值类型为x0，即网络直接预测x0
                pred_xstart = process_xstart(model_output)  # MDM是直接到这里跑到这，说明
            else:
                pred_xstart = process_xstart( # 均值类型为噪声，即网络直接预测噪声
                    self._predict_xstart_from_eps(x_t=x, t=t, eps=model_output)
                )
            model_mean, _, _ = self.q_posterior_mean_variance( # 得到x0后去算x_{t-1}的均值，即后验均值 q(x_{t-1} | x_t, x_0)
    ...
    def _predict_xstart_from_xprev(self, x_t, t, xprev):
        ''' 
        给定 xt, t, x_{t-1}, 计算x0
        根据DDPM论文公式(7) mu_t = x_{t-1} = coef1*x0 + coef2*xt, 得x0 = x_{t-1}/coef1 - (coef2/coef1) * xt
        '''
    ...
    def p_sample(
        ...
        #### 这里是额外加的，即将给定的部分替换为条件值


        return {"sample": sample, "pred_xstart": out["pred_xstart"]} # 分别是x_{t-1}和x_0
    ...
    def p_sample_loop(
        ...
        for i, sample in enumerate(self.p_sample_loop_progressive( # 每一次返回一个sample={"sample":x_{t-1}, x0}
            model,
            shape,
            noise=noise,
            clip_denoised=clip_denoised,
            ...
            if dump_steps is not None and i in dump_steps:
                dump.append(deepcopy(sample["sample"])) # sample["sample"]就是x_{t-1}
            final = sample
        if dump_steps is not None:
            return dump
        return final["sample"] # 最后一个x_{t-1}就是x0
    ...
    def p_sample_loop_progressive(
        ...
        if init_image is not None:
            ...
            img = self.q_sample(init_image, my_t, img) # init_image是x0，img是生成的随机噪声，这里是执行一次性1000步加噪
        ...
        for i in indices: # 从999到0
            ...
            with th.no_grad():
                sample_fn = self.p_sample_with_grad if cond_fn_with_grad else self.p_sample # 是p_sample
                # 额外
                img = torch.where(model_kwargs['traj_mask_263'], model_kwargs['gt_motion'], img)
                out = sample_fn( # 返回x_{t-1}和x0
                    model,
                    img, # [1,263,1,196]
                    t,   # 第t个step，从999到0
                    clip_denoised=clip_denoised,
                ...
                yield out # 返回x_{t-1}和x0
                img = out["sample"] #上面返回的x_{t-1}
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        enc = model.model # MDM
        mask = model_kwargs['y']['mask'] # (b,1,1,196)
        ...
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
            noise = th.randn_like(x_start) # 生成与x0形状一样的高斯噪声
        x_t = self.q_sample(x_start, t, noise=noise) # 给数据集x0加t步噪声
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(x_t, self._scale_timesteps(t), **model_kwargs) # 这里执行MDM的前向 (b,263,1,196)
            ...
            terms["rot_mse"] = self.masked_l2(target, model_output, mask) # mean_flat(rot_mse) 真实动作和生成动作要一样

            target_xyz, model_output_xyz = None, None

            if self.lambda_rcxyz > 0.: # 位置loss
            ...
            if self.lambda_fc > 0.: # foot contact loss
                ...
                if self.data_rep == 'rot6d' and dataset.dataname in ['humanact12', 'uestc']:
                    ...
                    terms["fc"] = self.masked_l2(pred_vel,
                                                 ...
                                                 mask[:, :, :, 1:])
            if self.lambda_vel > 0.: # 速度velocity loss
                target_vel = (target[..., 1:] - target[..., :-1])
                model_output_vel = (model_output[..., 1:] - model_output[..., :-1]) # 相邻两帧之差表示速度
