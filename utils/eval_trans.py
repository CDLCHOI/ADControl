...
@torch.no_grad()        
def evaluation_vqvae(out_dir, val_loader, net, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, eval_wrapper, draw = True, save = True, savegif=False, savenpy=False, pearson_flag=False) : 
    net.eval()
    nb_sample = 0
    ...
    for batch in val_loader:
        ...
        # word_embeddings: b,num_words,300; pos_one_hots b,num_words,15; sent_len b; motion: b,196,263; m_length b.
        motion = motion.cuda()
        ...
        for i in range(bs):
            ...
            # 重构得到的motion
            pred_pose, loss_commit, perplexity, pearson_loss, codebook_rank = net(motion[i:i+1, :m_length[i]])
...
def calculate_activation_statistics(activations):
    ...
    mu = np.mean(activations, axis=0) # （512）
    cov = np.cov(activations, rowvar=False) # （512，512）
    return mu, cov
